<?php // historyRepairUtilities.php
/**
 * Created by PhpStorm.
 * User: devon
 * Date: 3/19/17
 * Time: 2:33 PM
 *
 * Tweekable script to manually repair the history data store.
 *
 * MY IDEA IS TO BE ABLE TO:
 *  - JUST UPDATE ONE DATE FOR ALL STOCKS
 *  - JUST UPDATE ONE STOCK
 *  - UPDATE A WEEKS WORTH OF DATA
 *  - UPDATE histories DATASTORE, TOO
 *  - DELETE SOME DATA
 *
 */

/* TEXT FROM historyCron.php:=================================================
...
/**
 *
 * @desc A test function to prove we can get a specific day for a specific stock.
 * -------not meant to be saved to the datastore; rather, just output to browser the data of one stock. 
 *
 * @param String $date  the day to get a quote about
 * @param String $symbolsString  a YQL-acceptible string of symbols to search for
 *
 * @return array $newArray An associative array of arrays of stock quotes, keyed by symbol
 */
function addOneDayToOneHistory(){
    global $historycollection;

    $stocklist = getSymbolsFromHistory();
    $quotes = getQuotesFromYQL($stocklist);//returns key=symbol, value=the data for the datastore: history
